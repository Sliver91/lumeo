...
export class GameScene extends Phaser.Scene {
    ...
    setOwner(cell, player, skipAnim = false) {
        ...
        if (player !== 0) {
            ...
            // .setOrigin(0.5, 0.5) place le centre de l'emoji sur le centre de la case
            // le -10 permet de compenser visuellement la perspective du losange
            const unit = this.add.text(0, -10, icon, { fontSize: `${this.tileSize * 0.5}px` }).setOrigin(0.5, 0.5);
            ...
            this.tweens.add({
                targets: unit,
                y: -13, // Ajusté suite au décalage initial de -10
                alpha: 0.7,
